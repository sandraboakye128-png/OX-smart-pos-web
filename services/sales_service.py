...
def create_multi_sale(cart_items, sale_datetime=None, selected_batches=None):
    """
    Creates a sale using either FIFO (if selected_batches is None) or the exact
    batches provided in the flat list `selected_batches`.

    Args:
        cart_items: List of dicts with keys "product", "qty", "discount".
        sale_datetime: Optional custom datetime string (format: "YYYY-MM-DD HH:MM:SS")
        selected_batches: Optional list of dicts with keys "batch_id" and "qty"
                          (flat list across all products).
    """
    conn = get_connection()
    cursor = conn.cursor()
    ...
    # ------------------------------------------------------------------
    # CASE 1: Selected batches provided (manual batch selection)
    # ------------------------------------------------------------------
    if selected_batches:
        # Group selected batches by product_id
        batches_by_product = {}
        for sb in selected_batches:
            ...
            batches_by_product[prod_id].append({
                "batch": batch,
                "qty": sb["qty"]
            })

        # Process each cart item (product)
        for item in cart_items:
            ...
            for b in prod_batches:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    sale_id, product_id, batch["batch_id"],
                    sell_qty, batch_cost, batch_selling, batch_profit
                ))
    ...
    # ------------------------------------------------------------------
    # CASE 2: FIFO (default, no selected_batches)
    # ------------------------------------------------------------------
    else:
        for item in cart_items:
            product = item["product"]
            ...
            for b in batches:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    sale_id, product_id, b["batch_id"],
                    sell_qty, b["cost_price"], selling_price, batch_profit
                ))
    ...
    cursor.execute("""
        ...
        WHERE id=?
    """, (total_subtotal, total_discount, grand_total, total_profit, sale_id))
